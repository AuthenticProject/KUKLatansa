...
function doPost(e) {
  ...
  if (action === 'pelanggaran_bulk') {
    return handlePelanggaranBulk(data);
  }
...
// -------------------------------------------------------------------------
// FUNGSI: Simpan pelanggaran massal dari Import Fingerprint (Lap. Log Absen)
// Dipanggil dengan action: 'pelanggaran_bulk'
// Setiap record: { idKaryawan, nama, jenis, poin, tempat, staf, waktu, detail }
// -------------------------------------------------------------------------
function handlePelanggaranBulk(data) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    let sheet = ss.getSheetByName('Pelanggaran');
    if (!sheet) {
      sheet = ss.insertSheet('Pelanggaran');
      sheet.appendRow(['Waktu', 'ID Karyawan', 'Nama', 'Jenis Pelanggaran', 'Poin', 'Tempat', 'Staf Penginput', 'Detail']);
      const hr = sheet.getRange('A1:H1');
      hr.setFontWeight('bold');
      hr.setBackground('#fee2e2');
      sheet.setFrozenRows(1);
    }

    const records = data.records;
    if (!records || !Array.isArray(records)) throw new Error('Data records tidak valid');
    ...
    return ContentService.createTextOutput(JSON.stringify({
      result: 'success',
      message: 'Berhasil menyimpan ' + rows.length + ' pelanggaran.'
    })).setMimeType(ContentService.MimeType.JSON);
